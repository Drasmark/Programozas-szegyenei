...
PriceCalculatorInterface *cheapest(list<PriceCalculatorInterface*> calc_list, unsigned int months)
{
	PriceCalculatorInterface *ch=calc_list.front();
	for (list<PriceCalculatorInterface*>::iterator it=calc_list.begin(); it!=calc_list.end(); ++it)
	{
		if ((*it)->getPrice(months) < ch->getPrice(months)) ch=*it;
	}
	return ch;
}
